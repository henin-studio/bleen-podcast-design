...
% ─── Colors — V8-6 palette (avril 2026, finale) ───
\definecolor{consoVieuxRose}{HTML}{C75B7A}  % Vieux rose (haut du gradient)
\definecolor{consoPiaf}{HTML}{E8396B}       % Rose Piaf (coeur du gradient)
\definecolor{consoCorail}{HTML}{FF4D6A}     % Corail (pied du gradient)
\definecolor{consoCreme}{HTML}{FFF5E1}      % Creme chaud (couleur titre)
\definecolor{consoNoir}{HTML}{0A0A0A}       % Noir (contour + text-shadow)
\definecolor{consoNavy}{HTML}{221C46}       % Navy profond (historique + texte principal)
\definecolor{consoPrune}{HTML}{3D1A3E}      % Prune (historique)
\definecolor{consoBordeaux}{HTML}{8B2252}   % Bordeaux doux
% Palette historique (reference)
...
\newcommand{\sectionrule}{%
  ...
  \noindent\begin{tikzpicture}
    \shade[left color=consoNavy, right color=consoPiaf] (0,0) rectangle (\textwidth, 2.5pt);
  \end{tikzpicture}%
  \vspace{6pt}%
}
...
\tcbset{
  ...
  highlight card/.style={
    card,
    borderline north={2pt}{0pt}{consoNavy!70!consoPiaf},
  }
}
...
\begin{tikzpicture}[remember picture, overlay]
  % Background gradient — V8-6 (vieux rose -> rose Piaf -> corail)
  \shade[top color=consoVieuxRose, middle color=consoPiaf, bottom color=consoCorail]
    (current page.north west) rectangle ([yshift=-12cm]current page.north east);
  \fill[consoNavy]
    ([yshift=-12cm]current page.north west) rectangle (current page.south east);
\end{tikzpicture}

\vspace*{0.8cm}

\begin{center}
  {\knewave\fontsize{54}{54}\selectfont\color{consoCreme}Deconsommation}\\[8pt]
  {\Large\color{consoCreme}\bfseries Cover Art — Design System}\\[4pt]
  {\small\color{consoCreme!70}Version 2.2 — Avril 2026}\\[2pt]
...
\begin{center}
  \begin{tcolorbox}[enhanced, colback=consoNoir, colframe=consoNoir, arc=3pt, width=6.5cm, halign=center, boxrule=0pt, left=6pt, right=6pt, top=4pt, bottom=4pt]
    {\small\color{consoCreme}\bfseries PISTE RETENUE — V8-6}
  \end{tcolorbox}
\end{center}
...
\begin{center}
  {\footnotesize\color{consoCreme!60}Vieux rose $\rightarrow$ rose Piaf $\rightarrow$ corail · Illustration « Joyeuse fond rose »\\[2pt]Titre Knewave 1 ligne · Contour noir + text-shadow (style LGD/Vélotaf)}
\end{center}
...
\midrule
Nom du podcast & \textbf{Deconsommation} (retenu avril 2026, feedback Olivier De Schutter) \\
...
Univers visuel & Illustration flat stylisée, gradient navy $\rightarrow$ prune $\rightarrow$ vieux rose, accent rose Piaf \\
\bottomrule
\end{tabularx}
\end{tcolorbox}
...
\subsection{Gradient de la cover — V8-6}

Le fond utilise un \textbf{dégradé diagonal 135°} en trois étapes : vieux rose $\rightarrow$ rose Piaf $\rightarrow$ corail. Palette solaire, expressive, assumée féminine sans être mièvre — le ton est celui de la joie franche et de l'énergie. Cohérent avec la famille Bleen : Vélotaf (rouge-orange), La Grande Démission (indigo-rouge), Deconsommation (vieux rose $\rightarrow$ corail).

\begin{tcolorbox}[card]
\begin{center}
\begin{tikzpicture}
  \shade[top color=consoVieuxRose, middle color=consoPiaf, bottom color=consoCorail] (0,0) rectangle (14,3);
  \node[font=\small\bfseries, text=consoCreme, anchor=west] at (0.4, 2.5) {Vieux rose};
  \node[font=\tiny\ttfamily, text=consoCreme, anchor=west] at (0.4, 2.1) {\#C75B7A};
  \node[font=\small\bfseries, text=consoCreme, anchor=center] at (7, 1.5) {Rose Piaf \#E8396B};
  \node[font=\small\bfseries, text=consoCreme, anchor=east] at (13.6, 0.5) {Corail \#FF4D6A};
\end{tikzpicture}
\end{center}
\end{tcolorbox}
...
CSS: background: linear-gradient(135deg,
       #C75B7A, #E8396B, #FF4D6A);
\end{verbatim}

\subsection{Couleurs principales}

\noindent
\colorswatch{consoVieuxRose}{Vieux rose}{\#C75B7A}\hspace{18pt}%
\colorswatch{consoPiaf}{Rose Piaf}{\#E8396B}\hspace{18pt}%
\colorswatch{consoCorail}{Corail}{\#FF4D6A}

\vspace{6pt}\noindent
\colorswatch{consoCreme}{Creme chaud}{\#FFF5E1}\hspace{18pt}%
\colorswatch{consoNoir}{Noir contour}{\#0A0A0A}\hspace{18pt}%
\colorswatch{consoNavy}{Navy (historique)}{\#221C46}
...
\midrule
Vieux rose & \texttt{\#C75B7A} & Haut du gradient, adoucit le départ \\
Rose Piaf & \texttt{\#E8396B} & Cœur du gradient, dominant visuel, rappel Édith Piaf \\
Corail & \texttt{\#FF4D6A} & Pied du gradient, chaleur, énergie pop \\
Crème chaud & \texttt{\#FFF5E1} & Couleur du titre Knewave \\
Noir (contour) & \texttt{\#0A0A0A} & Stroke + text-shadow du titre (style LGD/Vélotaf) \\
Navy profond & \texttt{\#221C46} & Alternative fond sombre pour variantes, typo secondaire \\
\bottomrule
\end{tabularx}
\end{tcolorbox}
...
\begin{itemize}[nosep]
  \item \textbf{60\%} — Gradient vieux rose–Piaf–corail (fond dominant) + illustration
  \item \textbf{30\%} — Crème \texttt{\#FFF5E1} pour la typographie
  \item \textbf{10\%} — Noir \texttt{\#0A0A0A} en contour/ombre (garantit la lisibilité sur le rose)
\end{itemize}
\end{tcolorbox}

\begin{tcolorbox}[card, title=Interdits, colframe=consoPiaf!30]
\begin{itemize}[nosep]
  \item Pas de gris moyen (\#888, \#999) — la palette est franche et chaleureuse
  \item Pas de pastels — saturation $>$ 30\%
  \item Pas de noir dominant — le noir est réservé au contour/ombre du titre
  \item Pas de vert saturé — éviter le cliché « écolo green »
  \item Pas de bleu ciel — palette historique abandonnée
  \item Pas de jaune solaire en dominant — réservé aux variantes secondaires
\end{itemize}
\end{tcolorbox}

\subsection{Palette historique (v1 — abandonnée)}

\begin{tcolorbox}[card]
Palette bleue-rose-jaune initialement proposée (bleu \texttt{\#00B4F0}, rose \texttt{\#FF4D6A}, jaune \texttt{\#FFDC37}). Abandonnée après le feedback Olivier d'avril 2026. La palette V8-6 (vieux rose $\rightarrow$ rose Piaf $\rightarrow$ corail) remplace entièrement ces couleurs.
\end{tcolorbox}
...
\subsection{Aperçu du titre « Deconsommation » sur la cover}
...
\begin{tikzpicture}
  \shade[top color=consoVieuxRose, middle color=consoPiaf, bottom color=consoCorail]
    (0,0) rectangle (12,12);
  \node[anchor=north, font=\knewave\Huge, text=consoCreme] at (6, 10.5) {Deconsommation};
  \node[anchor=north, font=\tiny, text=consoCreme!85] at (6, 9.2) {1 ligne · Knewave · contour noir + text-shadow};
\end{tikzpicture}
\end{center}
...
\midrule
Mot titre & \textbf{Deconsommation} (1 seul mot, 1 ligne, contrainte Olivier) \\
Police titre & Knewave (Google Fonts, cursive brush) \\
Couleur titre & Crème chaud \texttt{\#FFF5E1} \\
Stroke & Contour noir \texttt{rgba(0,0,0,0.45)} — 2px (écran) / 24px (3000px) \\
Text-shadow & Ombre portée multi-directionnelle noire 35\% + glow noir 10px \\
Paint-order & \texttt{stroke fill} \\
Letter-spacing & $-0.015$em \\
Taille écran & \texttt{clamp(22px, 8.2vw, 36px)} \\
Padding horizontal & $2.5$\% de chaque côté du mot \\
White-space & \texttt{nowrap} — interdit le passage à 2 lignes \\
\bottomrule
\end{tabularx}
\end{tcolorbox}
...
Mêmes principes que Vélotaf et La Grande Démission, adaptés à la palette V8-6 :

\begin{enumerate}[nosep]
  \item \textbf{Remplissage} : crème \texttt{\#FFF5E1} — le titre reste lumineux sur le fond rose
  \item \textbf{Contour (stroke)} : noir semi-transparent \texttt{rgba(0,0,0,0.45)} — découpe le titre sur l'illustration
  \item \textbf{Paint-order} : \texttt{stroke fill} — contour peint sous le remplissage
  \item \textbf{Text-shadow multi-directionnelle} : 4 ombres noires à $\pm$2px + glow noir 10px — garantit la lisibilité partout sur la cover
  \item \textbf{Tagline optionnelle} : police Caveat (handwritten), crème à 90\% d'opacité
  \item \textbf{Une seule ligne imperative} : letter-spacing négatif obligatoire sur « Deconsommation »
\end{enumerate}
...
\subsection{Personnage : « Joyeuse, fond rose »}

\begin{itemize}[nosep]
  \item Jeune adulte, \textbf{rire éclatant} (bouche ouverte, yeux fermés de joie), allongé
  \item Entourée d'une accumulation chromatique d'objets de consommation (sneakers, smartphones, tasses, vêtements), tous dans la palette rose–corail–crème
  \item Symbolique : l'absurdité de l'accumulation, mais en mode joyeux et assumé — pas de moralisation
  \item Expression solaire, contagieuse — le podcast qui donne envie, pas qui culpabilise
  \item Cadrage serré : le visage occupe $\sim$30\% de la hauteur, le reste est rempli d'objets
\end{itemize}
...
\midrule
\texttt{v6-joyeuse-fond-rose.png} & Image source retenue (V8-6) \\
Chemin & \texttt{images/used/v6/v6-joyeuse-fond-rose.png} \\
Dimensions source & 600 × 371 px (à upscaler pour l'export 3000px) \\
Placement CSS & \texttt{top:0; left:-2\%; width:140\%; height:140\%; object-fit:cover} \\
Filtre & \texttt{drop-shadow(0 4px 16px rgba(0,0,0,0.3))} \\
Overlay haut & Gradient vieux rose $\rightarrow$ transparent sur les 38\% supérieurs (lisibilité titre) \\
\bottomrule
\end{tabularx}
\end{tcolorbox}

\subsection{Alternatives disponibles}

\begin{tcolorbox}[card]
\rowcolors{2}{white}{tableRowAlt}
\begin{tabularx}{\textwidth}{@{}lX@{}}
\toprule
\tableheader Fichier & Usage \\
\midrule
\texttt{v6-souriante-hoodie-rose.png} & Sourire franc, hoodie rose (V8-4, V8-11) \\
\texttt{v6-liberation-bras-ouverts.png} & Bras levés, fond noir (V8-1, V8-7 charbon, V8-8, V8.01-*) \\
\texttt{v6-paisible-yeux-fermes.png} & Yeux fermés, sérénité (V8-3, V8-12) \\
\texttt{v6-joyeuse-wave-rose-jaune.png} & Main levée, fond mixte (V8-2) \\
\texttt{v6-joyeuse-wave-fond-noir.png} & Main levée, fond noir (V8-5, V8-9) \\
\bottomrule
\end{tabularx}
\end{tcolorbox}
...
\begin{itemize}[nosep]
  \item Vérifier l'absence de halo ou aliasing sur le pourtour du personnage
  \item Préserver la saturation d'origine — la cohérence rose/corail de l'illustration supporte le gradient V8-6
  \item Assurer que le rire est bien centré dans le tiers médian vertical (zone lisible)
\end{itemize}
...
\begin{tikzpicture}[scale=0.55]
  % Cover frame
  \shade[top color=consoNavy, middle color=consoPrune, bottom color=consoVieuxRose] (0,0) rectangle (14,14);
...
\midrule
Background & \texttt{cover-*-bg.png} & Gradient navy $\rightarrow$ prune $\rightarrow$ vieux rose + vignette & $\sim$1.5 MB \\
...
\begin{itemize}[nosep]
  ...
  \item Ombre portée pour lisibilité sur le gradient vieux rose–Piaf–corail
...
\begin{itemize}[nosep, leftmargin=16pt]
  \doitem{Fond gradient vieux rose–Piaf–corail comme élément dominant}
...
\begin{itemize}[nosep, leftmargin=12pt]
  ...
  \dontitem{Noir dominant (la palette Deconsommation est vieux rose–Piaf–corail)}
...
\begin{itemize}[nosep, leftmargin=12pt]
  \dontitem{Climax n'a pas de charte couleur fixe (nous oui : vieux rose–Piaf–corail)}
...
% ═══════════════════════════════════════════
% 14. ÉVOLUTION V4 -> V8 — SOBRE & SEXY -> DECONSOMMATION
% ═══════════════════════════════════════════

\section{Évolution V4 $\rightarrow$ V8 — Sobre \& Sexy $\rightarrow$ Deconsommation}
\sectionrule

\begin{tcolorbox}[highlight card, title=Contexte — Historique des pivots]
Cette section documente les pivots successifs qui ont mené à la direction V8-6 retenue.

\textbf{Pivot 1 (avril 2026, mi-mois)} — Les covers V1$\rightarrow$V3 (absurde, bleu-rose, punk-pop) sont mises de côté au profit d'une direction \textbf{``Sobre \& Sexy''}. Verbatim Olivier : ``Make the transition sexy.''

\textbf{Pivot 2 (avril 2026, fin de mois)} — Après réflexion, ``La Vie en Sobre'' est abandonné (``trop alcooliques anonymes''). Le titre retenu devient \textbf{Deconsommation} — plus explicite, essentiel pour un podcast. Contrainte : \textbf{une seule ligne}.

\textbf{Pivot 3 (choix final)} — Sur la base de V8.01 (tests palette historique), la direction finale revient vers le rose dominant avec V8-6 : gradient vieux rose $\rightarrow$ rose Piaf $\rightarrow$ corail, illustration « Joyeuse fond rose » (rire éclatant). Le rose n'est plus perçu comme un risque de genre, mais comme une signature solaire assumée — cohérent avec Vélotaf (rouge-orange) et La Grande Démission (indigo-rouge).
\end{tcolorbox}
...
4 noms préférés (avril mi-mois) & La Vie en Sobre, Juste Assez, Zéro Superflu, La Déconsommation \\
Nom final (avril fin) & \textbf{Deconsommation} seul, sur une ligne \\
Rose dominant $\rightarrow$ accent & Risque de genre (``trop girly'') : le rose devient un accent, pas le fond \\
\bottomrule
\end{tabularx}
\end{tcolorbox}
...
\subsection{Évolution de la palette V6 $\rightarrow$ V7 $\rightarrow$ V8-6}

La palette ``Piaf'' a été introduite en V6/V7 (rose dominant). Le pivot V8 la rééquilibre : le rose Piaf reste présent mais comme \textbf{accent} — le fond devient un dégradé navy/prune/vieux rose qui évite la perception genrée.
...
\begin{itemize}[nosep]
  ...
  \item Gradient de fond (Piaf triple, bordeaux, noir $\rightarrow$ Piaf, ou solaire ?)
